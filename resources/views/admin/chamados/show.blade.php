...
<section class="mt-6 grid gap-6 xl:grid-cols-[1fr_380px]">
    ...
    <aside class="space-y-4">
        <form method="POST" action="{{ route('admin.chamados.responder', $chamado->numero) }}" enctype="multipart/form-data" class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <textarea name="mensagem" rows="6" required minlength="3" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 @error('mensagem') border-red-500 @enderror">{{ old('mensagem') }}</textarea>
            <label class="mt-3 flex items-center gap-2 text-sm font-semibold text-gray-600">
                <input type="hidden" name="interno" value="0">
                <input type="checkbox" name="interno" value="1" @checked(old('interno')) class="h-4 w-4 rounded accent-amber-500">
                Nota interna
            </label>
            ...
            <button type="submit" class="mt-3 w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">Enviar resposta</button>
        </form>
